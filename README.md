...
- **Learned daily limits with pace forecast** — since Google doesn't publish per-key RPD limits, the proxy learns the request count that tripped a daily 429 and forecasts, in `/metrics`, when each model will hit it again at the current request rate. RPD is scoped per key (per Google Cloud project), so the learned limit is per key too — the dashboard and `/metrics` multiply it by the number of active keys (`pooled_daily_limit`) for the pool-wide picture. Tokens-per-minute (TPM) limits are learned the same way from the first TPM 429, with their own pooled figure and a `tokens_last_minute` gauge. Nothing is hardcoded, so if Google changes a limit, the next 429 relearns it.
